...
if(!empty($_POST) && !empty($_POST['month'])){
	$url = "https://hana-admin.commsad.com/hana/cron/mobwithad_data.php?postman=postman&date=".$_POST['month'];
	//pre($url);
	$ch = curl_init();
	...
	//pre(curl_getinfo($ch));//마지막 http 전송 정보 출력
	//pre(curl_errno($ch));//마지막 에러 번호 출력
	///pre(curl_error($ch));//현재 세션의 마지막 에러 출력
