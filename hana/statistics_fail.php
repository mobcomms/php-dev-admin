...
## 환경설정
define('_title_', '광고 호출 실패');
define('_Menu_', 'manage');
define('_subMenu_', 'fail');
...
$sql="
	SELECT count(*) cnt FROM ckd_ad_fail
				...
				<div class="table-responsive table-wrap">
					<table class="table table-hover mb30" style="border:1px solid #b0b0b0;" id="jb-table">
						...
						<?php
							if(!empty($ret)) {
							foreach ($ret as $row){
								switch($row['user_app_os']){
									case "A" : $user_app_os = "Android";break;
									case "I" : $user_app_os = "iOS";break;
									default : $user_app_os = "-";break;
								}
						?>
						<tr>
							<td><?=$row['idx']?></td>
