...
if($keyword){
    ...
    if($search_type){
        $subQry.=" AND ";
		switch($search_type){
			case "01" : $subQry.="(question like :question or question like :question2)";break;
			case "02" : $subQry.="(reg_user like :question or reg_user like :question2)";break;
		}
        $ckd_param[":question"] = "%".$keyword."%";
        $ckd_param[":question2"] = "%".$keyword2."%";
...
$param .= "&target={$target}";
if($os_type == "A"){
	$subQry .= " AND user_app_os = 'A'";
}else if($os_type == "I"){
	$subQry .= " AND user_app_os = 'I'";
}
							...
							<div class="pull-left" style="margin-bottom: 10px;width: 100%;">
								<div class="col-sm-12">
									...
									<label style="padding-right: 7px;cursor: pointer">
										...
										<div class="pull-left" style="padding-top:5px;padding-left: 3px;">앱 오류 및 건의 사항</div>
									</label>
									<label style="padding-right: 7px;cursor: pointer">
								...
								<label style="margin: 4px 0 0 20px">
									<div class="pull-left"><input style="width:20px;height:20px;accent-color: darkcyan;" type="radio" name="os_type" id="os_type" onclick="display_toggle('');" <?=empty($os_type)?"checked":""?> value=""></div>
									<div class="pull-left" style="padding-top:5px;padding-left:3px;">전체</div>
								</label>
								<label style="margin: 4px 0 0 20px">
									<div class="pull-left"><input style="width:20px;height:20px;accent-color: darkcyan;" type="radio" name="os_type" id="os_type_a" onclick="display_toggle('A');" <?=($os_type=="A")?"checked":""?> value="A"></div>
									<div class="pull-left" style="padding-top:5px;padding-left:3px;">Android</div>
								</label>
								<label style="margin: 4px 0 0 20px">
									<div class="pull-left"><input style="width:20px;height:20px;accent-color: darkcyan;" type="radio" name="os_type" id="os_type_i" onclick="display_toggle('I');" <?=($os_type=="I")?"checked":""?> value="I"></div>
									<div class="pull-left" style="padding-top:5px;padding-left:3px;">iOS</div>
								</label>

								<div style="display: inline-block;" class="pull-right">
									<button type="button" class="btn btn-success" style="margin-left:10px;" id="ExcelDown" onclick="return false">Excel 다운</button>
								</div>
						...
						<div class="table-responsive">
							<table class="table table-hover mb30 member_table" id="Excel">
								...
								<tr>
									...
									<th class="col-md-1" style="width: 70px;">key값</th>
									<th class="col-md-1" style="width: 70px;">운영체제</th>
									<th class="col-md-1" style="width: 280px;">ADID</th>
									<th class="col-md-1" style="width: 200px;">userKey</th>
								...
								<?php if(empty($result) || !is_array($result)){?>
									<tr><td colspan="15">문의 리스트가 없습니다.</td></tr>
								<?php
								}else{
									foreach( $result AS $res){
										...
										$user_app_os = "";
										if($res['user_app_os'] == "A"){
											$user_app_os = "AOS";
										}else if($res['user_app_os'] == "I"){
											$user_app_os = "IOS";
										}
									?>
									<tr>
										...
										<td><?=$res['regdate']?></td>
										<td><?=empty($res['editdate'])?"":$res['editdate']?></td>
...
//Explorer 환경에서 다운로드
	...
	$(".btn-warning").click(function(){
		...
		if(chk_seq.length > 0){
			<?php if(empty($target)){ ?>
			centerOpenWindow('upd_board_inquiry.php', 'upd_board_inquiry', '700', '569', '', 'N');
			<?php }else{ ?>
			centerOpenWindow('upd_board_inquiry_sdk.php', 'upd_board_inquiry', '700', '651', '', 'N');
			<?php } ?>
		}else{
