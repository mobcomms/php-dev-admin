...
if(!empty($_POST) && !empty($_POST['month'])){
	$url = "http://paybooc-admin.commsad.com/cron/mobwithad_data.php?postman=postman&date=".$_POST['month'];
	$ch = curl_init();
	...
	$url = "http://paybooc-admin.commsad.com/cron/adjustment_data.php?postman=postman&date=".$_POST['month'];
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url);
	curl_setopt($ch, CURLOPT_TIMEOUT, 10);
	curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 3);
