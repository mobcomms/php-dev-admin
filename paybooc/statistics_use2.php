...
## 환경설정
define('_title_', '집계 방식 2 : 고정 CPC');
define('_Menu_', 'manage');
define('_subMenu_', 'use2');
...
switch($os_type){
	case "1" :
		...
		,IFNULL(CSTATS1.click_num, 0) AS coupang_click_num1
		,IFNULL(CSTATS1.click_num, 0) * 3.5 AS coupang_order1
	...
	case "2" :
		...
		,IFNULL(CDS6.click_num, 0) + IFNULL(CDS36.click_num, 0) AS mw_click6
		,IFNULL(CDS6.click_num, 0)*7 + IFNULL(CDS36.click_num, 0)*7 AS mw_exhs6
	...
	$sql.="
		...
		,IFNULL(CSTATS2.click_num, 0) AS coupang_click_num2
		,IFNULL(CSTATS2.click_num, 0) * 3.5 AS coupang_order2
		,0 AS coupang_cancel2
...
$sql.="
		...
		,IFNULL(CDS11.exhs_amt, 0) + IFNULL(CDS21.exhs_amt, 0) AS mw_exhs11
		,IFNULL(CDS12.exhs_amt, 0) + IFNULL(CDS22.exhs_amt, 0) AS mw_exhs12
		,IFNULL(CDS13.exhs_amt, 0) + IFNULL(CDS23.exhs_amt, 0) AS mw_exhs13
		,IFNULL(CDS14.exhs_amt, 0) + IFNULL(CDS24.exhs_amt, 0) AS mw_exhs14
		,IFNULL(CDS15.exhs_amt, 0) + IFNULL(CDS25.exhs_amt, 0) AS mw_exhs15
		,IFNULL(CDS16.exhs_amt, 0) + IFNULL(CDS26.exhs_amt, 0) AS mw_exhs16
		
	FROM ckd_day_app_stats CDMS
	LEFT JOIN ckd_day_ad_stats CDS1 ON CDS1.stats_dttm = CDMS.stats_dttm AND CDS1.service_tp_code='11'
	LEFT JOIN ckd_day_ad_stats CDS2 ON CDS2.stats_dttm = CDMS.stats_dttm AND CDS2.service_tp_code='12'
	LEFT JOIN ckd_day_ad_stats CDS3 ON CDS3.stats_dttm = CDMS.stats_dttm AND CDS3.service_tp_code='13'
	LEFT JOIN ckd_day_ad_stats CDS4 ON CDS4.stats_dttm = CDMS.stats_dttm AND CDS4.service_tp_code='14'
	LEFT JOIN ckd_day_ad_stats CDS5 ON CDS5.stats_dttm = CDMS.stats_dttm AND CDS5.service_tp_code='15'
	LEFT JOIN ckd_day_ad_stats CDS6 ON CDS6.stats_dttm = CDMS.stats_dttm AND CDS6.service_tp_code='16'
	LEFT JOIN ckd_day_ad_stats CDS7 ON CDS7.stats_dttm = CDMS.stats_dttm AND CDS7.service_tp_code='07'

	LEFT JOIN ckd_day_ad_stats CDS11 ON CDS11.stats_dttm = CDMS.stats_dttm AND CDS11.service_tp_code='01'
	LEFT JOIN ckd_day_ad_stats CDS12 ON CDS12.stats_dttm = CDMS.stats_dttm AND CDS12.service_tp_code='02'
	LEFT JOIN ckd_day_ad_stats CDS13 ON CDS13.stats_dttm = CDMS.stats_dttm AND CDS13.service_tp_code='03'
	LEFT JOIN ckd_day_ad_stats CDS14 ON CDS14.stats_dttm = CDMS.stats_dttm AND CDS14.service_tp_code='04'
	LEFT JOIN ckd_day_ad_stats CDS15 ON CDS15.stats_dttm = CDMS.stats_dttm AND CDS15.service_tp_code='05'
	LEFT JOIN ckd_day_ad_stats CDS16 ON CDS16.stats_dttm = CDMS.stats_dttm AND CDS16.service_tp_code='06'
	...
	LEFT JOIN ckd_day_ad_stats CDS50 ON CDS50.stats_dttm = CDMS.stats_dttm AND CDS50.service_tp_code='569'
	LEFT JOIN ckd_day_ad_stats CDS51 ON CDS51.stats_dttm = CDMS.stats_dttm AND CDS51.service_tp_code='570'
...
$make_array6 = array("mw_eprs1","mw_click1","mw_exhs1","mw_eprs2","mw_click2","mw_exhs2","mw_eprs3","mw_click3","mw_exhs3"
	,"mw_eprs4","mw_click4","mw_exhs4","mw_eprs5","mw_click5","mw_exhs5","mw_eprs6","mw_click6","mw_exhs6","covi_eprs1","covi_eprs2","covi_exhs1","covi_exhs2"
,"offerwall_exhs_amt_ori","mw_eprs_sdk","mw_click_sdk","mw_exhs_sdk","mw_ctr_sdk","mw_exhs11","mw_exhs12","mw_exhs13","mw_exhs14","mw_exhs15","mw_exhs16","coupang_order11","coupang_order12");
...
foreach($ret as $key => $row){
	...
	$coupang_income1 = $row['coupang_order1'] * $commission ;
	$coupang_income2 = $row['coupang_order2'] * $commission ;
	...
	$TOTAL['mw_exhs11'] += ($row['mw_exhs11']);
	$TOTAL['mw_exhs12'] += ($row['mw_exhs12']);
	$TOTAL['mw_exhs13'] += ($row['mw_exhs13']);
	$TOTAL['mw_exhs14'] += ($row['mw_exhs14']);
	$TOTAL['mw_exhs15'] += ($row['mw_exhs15']);
	$TOTAL['mw_exhs16'] += ($row['mw_exhs16']);

    $TOTAL['covi_eprs1'] += ($row['covi_eprs1']);
    $TOTAL['covi_exhs1'] += ($row['covi_exhs1']);
    $TOTAL['covi_eprs2'] += ($row['covi_eprs2']);
    $TOTAL['covi_exhs2'] += ($row['covi_exhs2']);
	...
	$TOTAL['coupang_order11'] += ($row['coupang_order11']);
	$TOTAL['coupang_order12'] += ($row['coupang_order12']);
	...
	if($type=="3M" || $type=="6M"){
		...
		$M_TOTAL[$month]['mw_exhs3'] += ($row['mw_exhs3']);

		$M_TOTAL[$month]['mw_click4'] += $row['mw_click4'];
		$M_TOTAL[$month]['mw_exhs4'] += ($row['mw_exhs4']);

		$M_TOTAL[$month]['mw_click5'] += $row['mw_click5'];
		$M_TOTAL[$month]['mw_exhs5'] += ($row['mw_exhs5']);

		$M_TOTAL[$month]['mw_click6'] += $row['mw_click6'];
		$M_TOTAL[$month]['mw_exhs6'] += ($row['mw_exhs6']);

		$M_TOTAL[$month]['mw_exhs11'] += ($row['mw_exhs11']);
		$M_TOTAL[$month]['mw_exhs12'] += ($row['mw_exhs12']);
		$M_TOTAL[$month]['mw_exhs13'] += ($row['mw_exhs13']);
		$M_TOTAL[$month]['mw_exhs14'] += ($row['mw_exhs14']);
		$M_TOTAL[$month]['mw_exhs15'] += ($row['mw_exhs15']);
		$M_TOTAL[$month]['mw_exhs16'] += ($row['mw_exhs16']);

		$M_TOTAL[$month]['coupang_click_num1'] += $row['coupang_click_num1'];
		$M_TOTAL[$month]['coupang_income1'] += ($coupang_income1);
		$M_TOTAL[$month]['coupang_click_num1'] += $row['coupang_click_num2'];
		$M_TOTAL[$month]['coupang_income2'] += ($coupang_income2);
        ...
        $M_TOTAL[$month]['covi_exhs1'] += ($row['covi_exhs1']);
        $M_TOTAL[$month]['covi_eprs2'] += ($row['covi_eprs2']);
        $M_TOTAL[$month]['covi_exhs2'] += ($row['covi_exhs2']);

    }

	$total_sales = $row['mw_exhs11']+$row['mw_exhs12']+$row['mw_exhs13']+$row['mw_exhs14']+$row['mw_exhs15']+$row['mw_exhs16']+$row['coupang_order11']+$row['coupang_order12']+$row['offerwall_exhs_amt']+$row['covi_exhs1']+$row['covi_exhs2'];
	$total_settlement = $row['mw_exhs1']+$row['mw_exhs2']+$row['mw_exhs3']+$row['mw_exhs4']+$row['mw_exhs5']+$row['mw_exhs6']+$coupang_income1+$coupang_income2+$row['offerwall_exhs_amt'];
	$total_profit = $total_sales - $total_settlement;

	$use_cnt = empty($row['use_cnt'])?0:number_format($row['use_cnt']);
	$css_total_profit = $total_profit<0 ?"color:red;":"color:blue;";
	...
	if($_SESSION['Adm']['id'] == "mango"){
		$html .= "<!-- 총 수익 --><td style='{$css_total_profit}font-weight:700;'>".number_format($total_profit)."</td>";
	}
	if($os_type != 2){
		$html .= "<!-- 일반광고(AOS) -->
...
//총수익
$total_sales_all = $TOTAL['mw_exhs11']+$TOTAL['mw_exhs12']+$TOTAL['mw_exhs13']+$TOTAL['mw_exhs14']+$TOTAL['mw_exhs15']+$TOTAL['mw_exhs16']+$TOTAL['coupang_order11']+$TOTAL['coupang_order12']+$TOTAL['offerwall_exhs_amt'];
$total_settlement_all = $TOTAL['mw_exhs1']+$TOTAL['mw_exhs2']+$TOTAL['mw_exhs3']+$TOTAL['mw_exhs4']+$TOTAL['mw_exhs5']+$TOTAL['mw_exhs6']+$TOTAL['coupang_income1']+$TOTAL['coupang_income2']+$TOTAL['offerwall_exhs_amt'];
$total_profit_all = $total_sales_all - $total_settlement_all;

$css_total_profit = $total_profit_all<0 ?"color:red;":"color:blue;";
...
<!--							<td style="color:blue;font-weight:700;">--><?//=number_format($total_sales_all)?><!--</td>-->
							...
							<?php if($_SESSION['Adm']['id'] == "mango"){ ?>
							<td style="<?=$css_total_profit?>font-weight:700;"><?=number_format($total_profit_all)?></td>
							<?php }?>
							<?php if($os_type != 2){ ?>
							<!-- 일반 광고(AOS) -->
					...
					if($type=="3M" || $type=="6M"){
						foreach ($M_TOTAL as $key=>$row){
							...
							$month_total_sales = $row['mw_exhs11']+$row['mw_exhs12']+$row['mw_exhs13']+$row['mw_exhs14']+$row['mw_exhs15']+$row['mw_exhs16']+$coupang_income1+$coupang_income2+$row['offerwall_exhs_amt'];
							$month_total_settlement = $row['mw_exhs1']+$row['mw_exhs2']+$row['mw_exhs3']+$row['mw_exhs4']+$row['mw_exhs5']+$row['mw_exhs6']+$row['coupang_income1']+$row['coupang_income2']+$row['offerwall_exhs_amt'];
							$month_total_profit = $month_total_sales - $month_total_settlement;

							$css_total_profit = $month_total_profit<0 ?"color:red;":"color:blue;";
						...
						<tr class="" style="background-color: #afe076;">
							...
							<?php if($_SESSION['Adm']['id'] == "mango"){ ?>
							<td style="<?=$css_total_profit?>font-weight:700;"><?=number_format($month_total_profit)?></td>
							<?php }?>
