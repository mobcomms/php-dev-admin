...
define('API_PATH', '/v2/providers/affiliate_open_api/apis/openapi/reports/clicks'); // 클릭
...
$curl_data = array();
function coupang_curl($subId, $page=0, $set_number_digits){
	global $curl_data;
		...
		if(empty($result['data']) === false) {
			...
			foreach($result['data'] as $key => $value) {
				...
				if(isset($curl_data[$value['date']]) === false) {
					$curl_data[$value['date']][$value['subId']]['click'] = 0;
				}
				$curl_data[$value['date']][$value['subId']]['click'] += $value['click'];
			}
		}
	}
	...
	if(!empty($result['data']) && count($result['data']) === 1000) {
		$page++;
		coupang_curl($subId, $page, $set_number_digits);
	}
}

foreach (API_SUB_ID as $subId){
	coupang_curl($subId, 0, $set_number_digits);
}
