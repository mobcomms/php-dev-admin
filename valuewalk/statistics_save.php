...
$sql="
	...
	,SUM(IFNULL(OUP.spot_point_cnt4, 0)) AS spot_point_cnt4
	,SUM(IFNULL(OUP.spot_sum_point4, 0)) AS spot_sum_point4
...
if(!empty($ret)) {
	foreach ($ret as $row) {
		...
		$row['tot_point_cnt'] = $row['spot_point_cnt1']+$row['spot_point_cnt2']+$row['spot_point_cnt3']+$row['spot_point_cnt4']+$row['offerwall_point_cnt'];
		$row['tot_sum_point'] = $row['spot_sum_point1']+$row['spot_sum_point2']+$row['spot_sum_point3']+$row['spot_sum_point4']+$row['offerwall_point_sum'];
		...
		$TOTAL['spot_point_cnt4'] += $row['spot_point_cnt4'];
		$TOTAL['spot_sum_point4'] += $row['spot_sum_point4'];

		$TOTAL['tot_point_cnt'] = $TOTAL['spot_point_cnt1']+$TOTAL['spot_point_cnt2']+$TOTAL['spot_point_cnt3']+$TOTAL['spot_point_cnt4'];
		$TOTAL['tot_sum_point'] = $TOTAL['spot_sum_point1']+$TOTAL['spot_sum_point2']+$TOTAL['spot_sum_point3']+$TOTAL['spot_sum_point4'];
		...
		$html .= "
			<tr class='{$dateColor}'>
				...
				<td class='show_aos'>".number_format($row['spot_point_cnt1'])."</td>
				<td class='show_aos'>".number_format($row['spot_sum_point1'])."</td>
				<td class='show_ios'>".number_format($row['spot_point_cnt2'])."</td>
				<td class='show_ios'>".number_format($row['spot_sum_point2'])."</td>
				<td class='show_aos'>".number_format($row['spot_point_cnt3'])."</td>
				<td class='show_aos'>".number_format($row['spot_sum_point3'])."</td>
				<td class='show_ios'>".number_format($row['spot_point_cnt4'])."</td>
				<td class='show_ios'>".number_format($row['spot_sum_point4'])."</td>
			 </tr>
		";
	}
				...
				<div class="table-responsive table-wrap">
					<table class="table table-hover mb30" style="border:1px solid #b0b0b0;" id="jb-table">
						...
						<tr>
							...
							<th colspan="2" class="show_aos">사다리광고(AOS)</th>
							<th colspan="2" class="show_ios">사다리광고(IOS)</th>
							<th colspan="2" class="show_aos">룰렛광고(AOS)</th>
							<th colspan="2" class="show_ios">룰렛광고(IOS)</th>
						</tr>
						<tr>
						...
						<tr style="background-color: #F2F5A9;">
							...
							<td class="show_aos"><?=number_format($TOTAL['spot_point_cnt1'])?></td>
							<td class="show_aos"><?=number_format($TOTAL['spot_sum_point1'])?></td>
							<td class="show_ios"><?=number_format($TOTAL['spot_point_cnt2'])?></td>
							<td class="show_ios"><?=number_format($TOTAL['spot_sum_point2'])?></td>
							<td class="show_aos"><?=number_format($TOTAL['spot_point_cnt3'])?></td>
							<td class="show_aos"><?=number_format($TOTAL['spot_sum_point3'])?></td>
							<td class="show_ios"><?=number_format($TOTAL['spot_point_cnt4'])?></td>
							<td class="show_ios"><?=number_format($TOTAL['spot_sum_point4'])?></td>
						</tr>
						<?php } ?>
						<?=$html?>
						</tbody>
	...
	function fnExcelReport(id, title) {
		...
		<table border='1px'>\
			...
			<tr>\
				...
				<th rowspan='2'>전체 적립금액</th>\
				<th colspan='2'>사다리광고(AOS)</th>\
				<th colspan='2'>사다리광고(IOS)</th>\
