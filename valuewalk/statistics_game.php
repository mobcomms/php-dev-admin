...
$sql="
	...
	FROM(
		...
		FROM ckd_game_stats WHERE game_main>0 GROUP BY stats_dttm
	) T
	LEFT JOIN (
		SELECT
		stats_dttm
		...
		FROM ckd_game_stats WHERE roulette>0 GROUP BY stats_dttm
	...
	LEFT JOIN(
		...
		FROM ckd_game_stats WHERE ladder>0 GROUP BY stats_dttm
	...
	LEFT JOIN(
		...
		FROM ckd_game_stats WHERE lotto>0 GROUP BY stats_dttm
	) T4 ON T.stats_dttm = T4.stats_dttm
	LEFT JOIN(
		SELECT
		stats_dttm
		,COUNT(roulette) AS roulette_DAU
		FROM ckd_game_stats WHERE roulette>0 AND roulette_ad>=1 GROUP BY stats_dttm
	) T5 ON T.stats_dttm = T5.stats_dttm
	LEFT JOIN(
		SELECT
		stats_dttm
		,COUNT(ladder) AS ladder_DAU
		FROM ckd_game_stats WHERE ladder>0 AND ladder_ad>=1 GROUP BY stats_dttm
	) T6 ON T.stats_dttm = T6.stats_dttm
	LEFT JOIN(
		SELECT
		stats_dttm
		,COUNT(lotto) AS lotto_DAU
		FROM ckd_game_stats WHERE lotto>0 AND lotto_ad>=1 GROUP BY stats_dttm
	) T7 ON T.stats_dttm = T7.stats_dttm
	LEFT JOIN(
		SELECT
		stats_dttm
		,COUNT(roulette) AS roulette_DAU2
		FROM ckd_game_stats WHERE roulette>0 AND roulette_ad>=2 GROUP BY stats_dttm
	) T8 ON T.stats_dttm = T8.stats_dttm
	LEFT JOIN(
		SELECT
		stats_dttm
		,COUNT(ladder) AS ladder_DAU2
		FROM ckd_game_stats WHERE ladder>0 AND ladder_ad>=2 GROUP BY stats_dttm
	) T9 ON T.stats_dttm = T9.stats_dttm
	LEFT JOIN(
		SELECT
		stats_dttm
		,COUNT(lotto) AS lotto_DAU2
		FROM ckd_game_stats WHERE lotto>0 AND lotto_ad>=2 GROUP BY stats_dttm
