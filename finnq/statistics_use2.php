...
switch($os_type){
	case "1" :
		...
		,IFNULL(CDS5.click_num, 0)*7 + IFNULL(CDS6.click_num, 0)*7 AS mw_exhs1

		,0 AS mw_click2
		,0 AS mw_exhs2

		,IFNULL(CSTATS5.click_num, 0) + IFNULL(CSTATS6.click_num, 0) AS coupang_click_num1
		,(IFNULL(CSTATS5.click_num, 0) + IFNULL(CSTATS6.click_num, 0)) * 3.5 AS coupang_order1
	...
	case "2" :
		...
		,IFNULL(CDS7.click_num, 0)*7 + IFNULL(CDS8.click_num, 0)*7 AS mw_exhs2
		...
		,0 AS coupang_click_num2
		,(IFNULL(CSTATS7.click_num, 0) + IFNULL(CSTATS8.click_num, 0)) * 3.5 AS coupang_order2
		,0 AS coupang_cancel2
	";
	break;
	default :
	$sql.="
		...
		,IFNULL(CSTATS7.click_num, 0) + IFNULL(CSTATS8.click_num, 0) AS coupang_click_num2
		,(IFNULL(CSTATS7.click_num, 0) + IFNULL(CSTATS8.click_num, 0)) * 3.5 AS coupang_order2
		,0 AS coupang_cancel2
	";
}
...
$sql.="
	...
	,IFNULL(CDS1.click_num, 0) + IFNULL(CDS2.click_num, 0) AS mw_click11
	,IFNULL(CDS1.click_num, 0)*7 + IFNULL(CDS2.click_num, 0)*7 AS mw_exhs11

	,IFNULL(CDS3.click_num, 0) + IFNULL(CDS4.click_num, 0) AS mw_click12
	,IFNULL(CDS3.click_num, 0)*7 + IFNULL(CDS4.click_num, 0)*7 AS mw_exhs12

	,IFNULL(CSTATS1.click_num, 0) + IFNULL(CSTATS2.click_num, 0) AS coupang_click_num11
	,(IFNULL(CSTATS1.click_num, 0) + IFNULL(CSTATS2.click_num, 0)) * 3.5 AS coupang_order11
	,0 AS coupang_cancel1

	,IFNULL(CSTATS3.click_num, 0) + IFNULL(CSTATS4.click_num, 0) AS coupang_click_num12
	,(IFNULL(CSTATS3.click_num, 0) + IFNULL(CSTATS4.click_num, 0)) * 3.5 AS coupang_order12
	,0 AS coupang_cancel12
...
$make_array6 = array("mw_eprs1","mw_click1","mw_exhs1","mw_eprs2","mw_click2","mw_exhs2","mw_eprs3","offerwall_exhs_amt_ori","mw_click11","mw_exhs11","mw_click12","mw_exhs12","coupang_click_num11","coupang_income11","coupang_click_num12","coupang_income12","coupang_order11","coupang_order12");
...
foreach($ret as $key => $row){
	...
	$TOTAL['mw_click11'] += $row['mw_click11'];
	$TOTAL['mw_exhs11'] += ($row['mw_exhs11']);
	$TOTAL['mw_click12'] += $row['mw_click12'];
	$TOTAL['mw_exhs12'] += ($row['mw_exhs12']);

	$TOTAL['coupang_click_num11'] += $row['coupang_click_num11'];
	$TOTAL['coupang_income11'] += ($coupang_income1);
	$TOTAL['coupang_click_num12'] += $row['coupang_click_num12'];
	$TOTAL['coupang_income12'] += ($coupang_income2);
	...
	if($type=="3M" || $type=="6M"){
		...
		$M_TOTAL[$month]['mw_click11'] += $row['mw_click11'];
		$M_TOTAL[$month]['mw_exhs11'] += ($row['mw_exhs11']);
		$M_TOTAL[$month]['mw_click12'] += $row['mw_click12'];
		$M_TOTAL[$month]['mw_exhs12'] += ($row['mw_exhs12']);

		$M_TOTAL[$month]['coupang_click_num11'] += $row['coupang_click_num11'];
		$M_TOTAL[$month]['coupang_income11'] += ($coupang_income1);
		$M_TOTAL[$month]['coupang_click_num11'] += $row['coupang_click_num12'];
		$M_TOTAL[$month]['coupang_income12'] += ($coupang_income2);

	}

	$total_sales = $row['mw_exhs11']+$row['mw_exhs12']+$row['coupang_order11']+$row['coupang_order12']+$row['offerwall_exhs_amt'];
	$total_settlement = $row['mw_exhs1']+$row['mw_exhs2']+$coupang_income1+$coupang_income2+($row['offerwall_exhs_amt']*0.7);
...
//총수익
$total_sales_all = $TOTAL['mw_exhs11']+$TOTAL['mw_exhs12']+$TOTAL['coupang_order11']+$TOTAL['coupang_order12']+$TOTAL['offerwall_exhs_amt'];
$total_settlement_all = $TOTAL['mw_exhs1']+$TOTAL['mw_exhs2']+$TOTAL['coupang_income1']+$TOTAL['coupang_income2']+($TOTAL['offerwall_exhs_amt']*0.7);
...
<!--							<td style="color:blue;font-weight:700;">--><?//=number_format($total_sales_all)?><!--</td>-->
					...
					if($type=="3M" || $type=="6M"){
						foreach ($M_TOTAL as $key=>$row){
							...
							$month_total_sales = $row['mw_exhs11']+$row['mw_exhs12']+$coupang_income1+$coupang_income2+$row['offerwall_exhs_amt'];
							$month_total_settlement = $row['mw_exhs1']+$row['mw_exhs2']+$row['coupang_income1']+$row['coupang_income2']+($row['offerwall_exhs_amt']*0.7);
