...
$sql="
	...
	,SUM(IFNULL(OUP.spot_user_cnt4, 0)) AS spot_user_cnt4
...
if(!empty($ret)) {
	foreach ($ret as $row) {
		...
		if($os_type == "A"){
			$row['tot_user_cnt'] = $row['spot_user_cnt1']+$row['spot_user_cnt3'];
			$row['tot_point_cnt'] = $row['spot_point_cnt1']+$row['spot_point_cnt3'];
			$row['tot_sum_point'] = $row['spot_sum_point1']+$row['spot_sum_point3'];
		}else if($os_type == "I"){
			$row['tot_user_cnt'] = $row['spot_user_cnt2']+$row['spot_user_cnt4'];
			$row['tot_point_cnt'] = $row['spot_point_cnt2']+$row['spot_point_cnt4'];
			$row['tot_sum_point'] = $row['spot_sum_point2']+$row['spot_sum_point4'];
		}else{
			$row['tot_user_cnt'] = $row['spot_user_cnt1']+$row['spot_user_cnt2']+$row['spot_user_cnt3']+$row['spot_user_cnt4'];
			$row['tot_point_cnt'] = $row['spot_point_cnt1']+$row['spot_point_cnt2']+$row['spot_point_cnt3']+$row['spot_point_cnt4'];
			$row['tot_sum_point'] = $row['spot_sum_point1']+$row['spot_sum_point2']+$row['spot_sum_point3']+$row['spot_sum_point4'];
		}
		...
		$TOTAL['spot_user_cnt4'] += $row['spot_user_cnt4'];
		...
		if($type=="3M" || $type=="6M"){
			...
			$M_TOTAL[$month]['spot_user_cnt4'] += $row['spot_user_cnt4'];
			$M_TOTAL[$month]['spot_point_cnt4'] += $row['spot_point_cnt4'];
			$M_TOTAL[$month]['spot_sum_point4'] += $row['spot_sum_point4'];
		...
		if($os_type != "I"){
			...
			$html .= "
				<td>".number_format($row['spot_user_cnt4'])."</td>
				<td>".number_format($row['spot_point_cnt4'])."</td>
				<td>".number_format($row['spot_sum_point4'])."</td>
			 </tr>
			";
		}
	}
			...
			<div class="row member_table .col-xs-12 .col-md-12" style="margin-top:8px">
				<div class="table-responsive table-wrap">
					<table class="table table-hover mb30" style="border:1px solid #b0b0b0;" id="jb-table">
						...
						<?php if($os_type != "I"){ ?>
							<th colspan="3">미션존 사다리(AOS)</th>
						<?php } if($os_type != "A"){ ?>
							<th colspan="3">미션존 사다리(IOS)</th>
						<?php } ?>
						<?php if($os_type != "I"){ ?>
							<th colspan="3">미션존 룰렛(AOS)</th>
						<?php } if($os_type != "A"){ ?>
							<th colspan="3">미션존 룰렛(IOS)</th>
						<?php } ?>
						</tr>
						<tr>
							<?php if($os_type != "I"){ ?>
						...
						<tr style="background-color: #F2F5A9;">
							...
							<?php } if($os_type != "A"){ ?>
							<td><?=number_format($TOTAL['spot_user_cnt4'])?></td>
							<td><?=number_format($TOTAL['spot_point_cnt4'])?></td>
							<td><?=number_format($TOTAL['spot_sum_point4'])?></td>
							<?php } ?>
	...
	function fnExcelReport(id, title) {
		...
		<table border='1px'>\
			...
			<tr>\
				...
				<th rowspan='2'>전체 적립 금액</th>";
		<?php if($os_type == 'A'){ ?>
		tab_text = tab_text + "<th colspan='3'>미션존 일반(AOS)</th>";
		tab_text = tab_text + "<th colspan='3'>미션존 쿠팡(AOS)</th>";
		<?php }else if($os_type == 'I'){ ?>
		tab_text = tab_text + "<th colspan='3'>미션존 일반(IOS)</th>";
		tab_text = tab_text + "<th colspan='3'>미션존 쿠팡(IOS)</th>";
		<?php }else{ ?>
		tab_text = tab_text + "<th colspan='3'>미션존 일반(AOS)</th>";
		tab_text = tab_text + "<th colspan='3'>미션존 일반(IOS)</th>";
		tab_text = tab_text + "<th colspan='3'>미션존 쿠팡(AOS)</th>";
		...
		tab_text = tab_text + "<th>유니크요청수</th>\
				<th>요청수</th>\
				<th>적립금</th>\
				<th>유니크요청수</th>\
				<th>요청수</th>\
...
//Explorer 환경에서 다운로드
	...
	$("#ExcelDown").click(function(){
		fnExcelReport('excel','적립통계 <?=empty($os_type)?"전체":($os_type=='A'?" AOS":" iOS")?> ');
	});
