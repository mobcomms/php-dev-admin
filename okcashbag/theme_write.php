					...
					<div class="panel panel-default">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								...
								<label class="theme_thumb_new col-sm-1 control-label" style="width:130px;margin-top:10px;">NEW 썸네일 <span class="asterisk"></span></label>
								<div class="theme_thumb_new fileupload fileupload-new" data-provides="fileupload" style="margin-top:10px;">
									<div class="input-append">
										<div class="uneditable-input">
											<i class="glyphicon glyphicon-file fileupload-exists"></i>
											<span class="fileupload-preview"></span>
										...
										<span class="btn btn-default btn-file">
											<span class="fileupload-new">찾아보기</span>
											<span class="fileupload-exists">Change</span>
											<input type="file" name="theme_thumb_new" />
										  </span>
										<a href="#" class="btn btn-default fileupload-exists" data-dismiss="fileupload">Remove</a>
										<?php if(!empty($idx) && file_exists($hpath.'theme_'.$idx.'_new.png') === true) { ?>
											<button type="button" class="btn btn-default" onclick="download_theme('image2')">다운로드</button>
										<?php } ?>
									</div>
								</div>

								<label class="theme_thumb_offerwall col-sm-1 control-label" style="width:130px;margin-top:10px;">오퍼월 썸네일 <span class="asterisk"></span></label>
								<div class="theme_thumb_offerwall fileupload fileupload-new" data-provides="fileupload" style="margin-top:10px;">
									<div class="input-append">
										<div class="uneditable-input">
											<i class="glyphicon glyphicon-file fileupload-exists"></i>
											<span class="fileupload-preview"></span>
										...
										<span class="btn btn-default btn-file">
											<span class="fileupload-new">찾아보기</span>
											<span class="fileupload-exists">Change</span>
											<input type="file" name="theme_thumb_offerwall" />
										  </span>
										<a href="#" class="btn btn-default fileupload-exists" data-dismiss="fileupload">Remove</a>
										<?php if(!empty($idx) && file_exists($offerwall_path.'theme_'.$idx.'.png') === true) { ?>
											<button type="button" class="btn btn-default" onclick="download_theme('image3')">다운로드</button>
										<?php } ?>
									</div>
								</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<select class="form-control pull-left border-input" name="partner_code" style="width:150px;margin-right:10px;" required>
									...
									<option value="01" <?=!empty($theme) && $theme['theme_partner_code']=="01"?"selected":""?> >구버전+오퍼월</option>
									<option value="02" <?=!empty($theme) && $theme['theme_partner_code']=="02"?"selected":""?> >오퍼월 전용</option>
								</select>
							</div>
						</div>
...
$(document).ready(function(){
	...
	<?php if(!empty($theme) && $theme['theme_partner_code']=="01"){?>
	$(".theme_thumb").show();
	$(".theme_thumb_new").show();
	$(".theme_thumb_offerwall").show();
	<?php }else{?>
	$(".theme_thumb").hide();
	$(".theme_thumb_new").hide();
	$(".theme_thumb_offerwall").show();
	<?php }?>

	$("select[name=partner_code]").change(function(){
		partner_code = $(this).val();

		if(partner_code == "01") {
			$(".theme_thumb").show();
			$(".theme_thumb_new").show();
			$(".theme_thumb_offerwall").show();
		}else if(partner_code == "02"){
			$(".theme_thumb").hide();
			$(".theme_thumb_new").hide();
			$(".theme_thumb_offerwall").show();
		}
