...
if($keyword){
	...
	$ocb_param[":question"] = "%".$keyword."%";
	$param .= "&keyword=".$keyword;
}
$sql="
	SELECT count(*) AS cnt FROM ocb_bbs_inquiry
	WHERE del_yn = 'N'
	{$subQry}
	ORDER BY seq DESC
";
//pre($sql);
$total = $NDO->getData($sql,$ocb_param);
...
";
//pre($sql);
$result = $NDO->fetch_array($sql,$ocb_param);
//pre($result);
							...
							<div class="pull-left" style="margin-bottom: 10px;width: 100%;">
								<div class="col-sm-11">
									...
									<label style="padding-right: 7px;cursor: pointer">
										...
										<div class="pull-left" style="padding-top:5px;padding-left: 3px;">돈키 이용 문의</div>
									</label>
									<label style="padding-right: 7px;cursor: pointer">
							...
							<table class="table table-hover mb30 member_table" id="Excel">
								...
								<?php
									...
									<tr>
										...
										<td><?=$res['seq']?></td>
										<td><?=empty($row_adid['adid'])?"":$row_adid['adid']?></td>
