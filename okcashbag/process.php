...
if(
	REFER($_POST['refpage'], '/okcashbag/board_brand_write.php') != true &&
	REFER($_POST['refpage'], '/okcashbag/board_brand_util_write.php') != true &&
	REFER($_POST['refpage'], '/okcashbag/board_banner_write.php') != true &&
	REFER($_POST['refpage'], '/okcashbag/board_promotion_write.php') != true &&
	REFER($_POST['refpage'], '/okcashbag/board_setting_write.php') != true &&
	REFER($_POST['refpage'], '/okcashbag/board_noti_write.php') != true &&
	REFER($_POST['refpage'], '/okcashbag/theme_write.php') != true &&
	REFER($_POST['refpage'], '/okcashbag/theme.php') != true &&
...
Switch($_POST['refpage']) {
	...
	default:    ## 이외 리다이렉트
		...
		Alert("권한이 없습니다");
		Loca("/okcashbag/login.php");
		exit;

	break;

	## 브랜드, 고정광고 ,프로모션 등록, 노티 추가
	Case '/okcashbag/board_brand_write.php':
	Case '/okcashbag/board_brand_util_write.php':
	Case '/okcashbag/board_banner_write.php':
	Case '/okcashbag/board_promotion_write.php':
	Case '/okcashbag/board_noti_write.php':
	Case '/okcashbag/board_setting_write.php':

		if($_POST['refpage'] =='/okcashbag/board_brand_write.php'){
			$go_page = "board_brand_write.php";
			$go_page_l = "board_brand.php";
		}else if($_POST['refpage'] =='/okcashbag/board_brand_util_write.php'){
			$go_page = "board_brand_util_write.php";
			$go_page_l = "board_brand_util.php";
		}else if($_POST['refpage'] =='/okcashbag/board_banner_write.php'){
			$go_page = "board_banner_write.php";
			$go_page_l = "board_banner.php";
		}else if($_POST['refpage'] =='/okcashbag/board_promotion_write.php'){
			$go_page = "board_promotion_write.php";
			$go_page_l = "board_promotion.php";
		}else if($_POST['refpage'] =='/okcashbag/board_noti_write.php'){
			$go_page = "board_noti_write.php";
			$go_page_l = "board_noti.php";
		}else if($_POST['refpage'] =='/okcashbag/board_setting_write.php'){
			$go_page = "board_setting_write.php";
			$go_page_l = "board_setting.php";
		}
		...
		if ($mode == 'DEL') {
			...
			$sql = "UPDATE ckd_bbs_okcash SET del_yn='Y' WHERE seq='{$idx}'";
			...
			if(empty($idx)){
				$sql = "select seq from ckd_bbs_okcash order by seq desc limit 1";
	...
	Case '/okcashbag/theme_write.php' : ## 테마등록
		...
		if($mode =='write'){
			...
			//썸네일 NEW
			if(isset($_FILES['theme_thumb_new']['name'])){
				if(!empty($_FILES['theme_thumb_new']['name'])){
					if($_FILES['theme_thumb_new']['error'] == '0') {
						$file = "theme_".$tdix."_new.png";

						if(!move_uploaded_file($_FILES['theme_thumb_new']['tmp_name'], $path.strtolower($file))) {
							$text = file_err_chk($_FILES['theme_thumb_new']['error']);
							exit($fn->loca("theme_write.php?idx=".$tdix, "NEW 썸네일 ".$text));
						}
					} else {
						$text = file_err_chk($_FILES['theme_thumb_new']['error']);
						exit($fn->loca("theme_write.php?idx=".$tdix, "NEW 썸네일 ".$text));
			...
			//오퍼월
			if(isset($_FILES['theme_thumb_offerwall']['name'])){
				if(!empty($_FILES['theme_thumb_offerwall']['name'])){
					if($_FILES['theme_thumb_offerwall']['error'] == '0') {
						$file = "theme_".$tdix.".png";

						if(!move_uploaded_file($_FILES['theme_thumb_offerwall']['tmp_name'], $path_offerwall.strtolower($file))) {
							$text = file_err_chk($_FILES['theme_thumb_offerwall']['error']);
							exit($fn->loca("theme_write.php?idx=".$tdix, "오퍼월 썸네일 ".$text));
						}
					} else {
						$text = file_err_chk($_FILES['theme_thumb_offerwall']['error']);
						exit($fn->loca("theme_write.php?idx=".$tdix, "오퍼월 썸네일 ".$text));
	...
	Case '/okcashbag/theme.php' : ## 테마삭제
		...
		if($mode == 'DEL') {
			...
			exit($fn->loca('/okcashbag/theme.php', '삭제되었습니다.'));
		}
