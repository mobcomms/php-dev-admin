...
if(!empty($_POST)){
	...
	if($method == 'upd'){
		...
		$sql="
			UPDATE ocb_bbs_inquiry SET answer=:answer, bbs_state=:bbs_state, editdate=NOW() 
		...
		$sql="
			UPDATE ocb_bbs_inquiry SET del_yn= 'Y' WHERE seq=:seq
		";
...
$sql="
	SELECT type, question, answer, bbs_state, regdate, reg_user, editdate FROM ocb_bbs_inquiry
	WHERE del_yn = 'N' AND seq=:seq
	ORDER BY seq DESC
";
//pre($sql);
...
switch($result['type']){
	case "01" : $inquiry_type = "돈키 이용 문의";break;
...
$sql="
	SELECT count(reg_user) cnt FROM ocb_bbs_inquiry
...
//ADID
$sql="
	SELECT adid FROM ckd_adid_info
	WHERE uuid = '{$result['reg_user']}'
";
$row_adid = $NDO->getData($sql);
			...
			<form method="POST" name="basicForm" id="basicForm" class="form-horizontal">
				...
				<div class="row">
					<div class="panel panel-default">
						...
						<div class="form-group">
							<label class=" col-sm-1 control-label" style="width: 200px;padding-top: 0;line-height: 35px;">문의한 아이디 :&nbsp;</label>
							<div class="col-sm-9">
								<?=$result['reg_user']?>
								<a href="member_list.php?key=user_uuid&keyword=<?=explode("@",$result['reg_user'])[0]?>" class="btn btn-primary">적립내역 보기</a>
								<?php if($user_cnt > 1){ ?>
									<a href="board_inquiry.php?search_type=02&keyword=<?=explode("@",$result['reg_user'])[0]?>" class="btn btn-warning">질문 <?=$user_cnt?>건 모아보기</a>
								<?php } ?>
							</div>
						</div>
						<div class="form-group">
							<label class=" col-sm-1 control-label" style="width: 200px;padding-top: 0">문의한 ADID :&nbsp;</label>
							<div class="col-sm-9"><?=empty($row_adid['adid'])?"":$row_adid['adid']?></div>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<label style="padding-right: 7px">
									<div class="pull-left"><input style="width:20px;height:20px;" type="checkbox" name="bbs_state" value="02" <?=(empty($result['bbs_state']) || $result['bbs_state']=='02')?'checked':''?> ></div>
...
<script>
	...
	$(document).ready(function(){
		$("input[name=content]").on("click", function(){
			var text = "";
			switch($(this).val()){
				case "1" : text = "안녕하세요. OK 캐시백 돈버는 키보드 운영진입니다.\n" +
					"전달 주신 내용으로는 정확한 문의 내용을 파악하기 어렵습니다.\n" +
					"발생한 오류  내용 전달해주시면,  확인해보도록 하겠습니다.\n" +
					"감사합니다.";
				break;
				case "2" : text = "안녕하세요. OK 캐시백 돈버는 키보드 운영진입니다.\n" +
					"많은 기대로 이용을 해 주셨을 텐데 이용에 불편을 드려 죄송합니다.\n" +
					"말씀해 주신 내용은 유관부서로 전달하였으며, 빠른 시일 내에 처리 될 수 있도록 노력하겠습니다.\n" +
					"감사합니다.";
				break;
				case "3" : text = "안녕하세요. OK 캐시백 돈버는 키보드 운영진입니다.\n" +
					"많은 기대로 이용을 해 주셨을 텐데 이용에 불편을 드려 죄송합니다.\n\n" +
					"첫번째 키보드 변경 방법입니다.\n" +
					"자판 오른쪽에 아래쪽 구석을 보시면  자판모양 아이콘이 있습니다.\n" +
					"이걸 클릭하시면 원래 키보드로 변경 가능합니다.\n\n" +
					"두번째 돈버는 키보드 OFF 처리 방법입니다.\n" +
					"휴대폰 설정 > 기본 키보드 및 추가 키보드설정 > 기본 키보드 > OK 캐시백 돈버는 키보드 설정 해제.\n" +
					"감사합니다.";
				break;
				case "4" : text = "안녕하세요. OK 캐시백 돈버는 키보드 운영진입니다.\n" +
					"많은 기대로 이용을 해 주셨을 텐데 이용에 불편을 드려 죄송합니다.\n\n" +
					"(주의) ※ OK Cashbag 앱이 로그인이 해제됩니다.\n" +
					"휴대폰 설정 > 어플리케이션 > \"OK Cashbag\" 검색후 클릭 > 저장공간 > 데이터삭제\n\n" +
					"하신후에 천지인(기본) 을 다시 변경 해보시기 바랍니다.\n" +
					"감사합니다." ;
				break;
				case "5" : text = "안녕하세요. OK 캐시백 돈버는 키보드 운영진입니다.\n" +
					"OK 캐시백 키보드에서는 회원분들에게 다양한 적립 혜택을 드리기 위해 광고가 노출되고 있으며,\n" +
