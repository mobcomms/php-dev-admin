...
if($idx){
	...
	if($bbs) {
		...
		$sql = "
			...
			WHERE service_tp_code='03' AND type='board_setting' AND board_idx = '{$idx}'
		";
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<form name="basicForm" id="basicForm" action="./process.php" class="form-horizontal" method="POST" enctype="multipart/form-data">
				...
				<input type="hidden" name="type" value="board_setting" />
				<div class="row">
					<div class="panel panel-default">
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								<input type="text" name="title" class="form-control" style="width:500px" value="<?=empty($bbs)?"":$bbs['title']?>" placeholder="타이틀은 띄워쓰기 포함 16글자를 권장 합니다.(타이틀,설명 중 1개는 필수 입력)" required />
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-1 control-label" style="width: 200px;" ><span class="asterisk">*</span> 설명 </label>
							<div class="col-sm-9">
								<input type="text" name="contents" class="form-control" style="width:500px" value="<?=empty($bbs)?"":$bbs['contents']?>" placeholder="타이틀은 띄워쓰기 포함 18글자를 권장 합니다.(타이틀,설명 중 1개는 필수 입력)" required />
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="pull-left input-group call  col-md-2" style="margin-right:20px;">
								<input type="text" class="form-control pull-left border-input" placeholder="yyyy-mm-dd" id="boardSdate" value="<?=empty($bbs)||$bbs['start_date']=="0000-00-00"?"":$bbs['start_date']?>" name="sdate" autocomplete="off" >
						...
						<div class="form-group">
							...
							<div class="pull-left input-group call  col-md-2" style="margin-right:20px;">
								<input type="text" class="form-control pull-left border-input" placeholder="yyyy-mm-dd" id="boardEdate" value="<?=empty($bbs)||$bbs['end_date']=="0000-00-00"?"":$bbs['end_date']?>" name="edate" autocomplete="off" >
								<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
							</div>
						...
						<div class="form-group">
							<label class="col-sm-1 control-label" style="width: 200px;"><span class="asterisk">*</span>  광고 이미지</label>
							<div class="col-sm-9">
						...
						<div class="form-group">
							<label class=" col-sm-1 control-label" style="width: 200px;">광고 링크</label>
							<div class="col-sm-9">
								<input type="text" name="url" class="form-control" style="width:700px" placeholder="딥링크는 앱내 해당 페이지로, URL을 입력하면 외부 브라우저를 통해 열리게 됩니다.(필수)"  value="<?=empty($bbs)?"":$bbs['url']?>" />
							</div>
						</div>

						<div class="form-group">
							<label class="col-sm-1 control-label" style="width: 200px;">상시노출</label>
							<div class="col-sm-10">
								<label style="padding-right: 7px">
									<div class="pull-left"><input type="checkbox" class="pull-left chk_box" name="default_yn" value="Y" id="default_yn" style="width:20px;height:20px;margin-left:10px;" <?=!empty($bbs) && $bbs['default_yn']=='Y'?'checked':''?> ></div>
									<div class="pull-left" style="padding-top: 5px;padding-left: 3px;">선택하면 시작 일시, 종료 일시는 비활성화 되며 상시 노출됩니다.</div>
								</label>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-10">
								<label style="padding-right: 7px">
									<div class="pull-left"><input type="checkbox" class="chk_box" name="display_yn" value="Y" style="width:20px;height:20px;margin-left:10px;" <?=!empty($bbs) && $bbs['display_yn']=='Y'?'checked':''?> ></div>
									<div class="pull-left" style="padding-top: 5px;padding-left: 3px;">체크 취소시 '상시노출' 체크상태여도 노출되지 않습니다.</div>
								</label>
							</div>
				...
				<div class="row">
					<div class="text-center">
						<div class="">
							...
							<?php }?>
							<button type="button" class="btn btn-default" onclick="location.href='board_setting.php' ">목록</button>
						</div>
					</div>
				</div>
			</form>
...
<script>
	function sendIt(){
		...
		if(!f.title.value && !f.contents.value) {
			alert("타이틀 또는 설명을 입력해 주세요.");
			return;
		}

		if(!f.default_yn.checked){
			if(!sdate) {
				alert("시작일시를 입력해 주세요.");
				$("[name=sdate]").focus();
				return;
...
<?php }?>
	...
	$(document).ready(function(){
		if($("#default_yn").is(":checked")){
			document.basicForm.sdate.disabled = true;
			document.basicForm.edate.disabled = true;
		}

	});

	$("#default_yn").click(function(){
		if($("#default_yn").is(":checked")) {
			document.basicForm.sdate.disabled = true;
			document.basicForm.edate.disabled = true;
		}else{
			document.basicForm.sdate.disabled = false;
			document.basicForm.edate.disabled = false;
		}
	})
