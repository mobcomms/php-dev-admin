...
$sql="
	...
	,SUM(IFNULL(OUP.spot_user_cnt1, 0)) + SUM(IFNULL(OUP.spot_user_cnt2, 0))  AS ad_aos_user
	,SUM(IFNULL(OUP.spot_point_cnt1, 0)) + SUM(IFNULL(OUP.spot_point_cnt2, 0))  AS ad_aos_cnt
	,SUM(IFNULL(OUP.spot_sum_point1, 0)) + SUM(IFNULL(OUP.spot_sum_point2, 0))  AS ad_aos_sum
	
	,SUM(IFNULL(OUP.spot_user_cnt3, 0)) + SUM(IFNULL(OUP.spot_user_cnt4, 0))  AS ad_ios_user
	,SUM(IFNULL(OUP.spot_point_cnt3, 0)) + SUM(IFNULL(OUP.spot_point_cnt4, 0)) AS ad_ios_cnt
	,SUM(IFNULL(OUP.spot_sum_point3, 0)) + SUM(IFNULL(OUP.spot_sum_point4, 0)) AS ad_ios_sum

	,(IFNULL(OUP.offerwall_user_cnt, 0)) offerwall_user_cnt
...
if(!empty($ret)) {
	foreach ($ret as $row) {
		...
		$row['tot_point_cnt'] = $row['ad_aos_cnt']+$row['ad_ios_cnt']+$row['offerwall_point_cnt'];
		$row['tot_sum_point'] = $row['ad_aos_sum']+$row['ad_ios_sum']+$row['offerwall_point_sum'];
		$row['tot_user_cnt'] = $row['ad_aos_user']+$row['ad_ios_user']+$row['offerwall_user_cnt'];
		...
		$TOTAL['offerwall_user_cnt'] += $row['offerwall_user_cnt'];

		$TOTAL['tot_point_cnt'] = $TOTAL['ad_aos_cnt']+$TOTAL['ad_ios_cnt']+$TOTAL['offerwall_point_cnt'];
		$TOTAL['tot_sum_point'] = $TOTAL['ad_aos_sum']+$TOTAL['ad_ios_sum']+$TOTAL['offerwall_point_sum'];
		$TOTAL['tot_user_cnt'] = $TOTAL['ad_aos_user']+$TOTAL['ad_ios_user']+$TOTAL['offerwall_user_cnt'];
		...
		$html .= "
			<tr class='{$dateColor}'>
				...
				<td>".number_format($row['offerwall_user_cnt'])."</td>
			...
			<div class="row member_table .col-xs-12 .col-md-12" style="margin-top:8px">
				<div class="table-responsive table-wrap">
					<table class="table table-hover mb30" style="border:1px solid #b0b0b0;" id="jb-table">
						<caption style="color:red; text-align: left;">※ 적립 실패건의 경우 API가 자동으로 재시도 하기때문에, 3일간 업데이트 합니다.</caption>
						<thead>
						<tr>
							<th rowspan="2">날짜</th>
						...
						<tr style="background-color: #F2F5A9;">
							...
							<td><?=number_format($TOTAL['offerwall_user_cnt'])?></td>
...
//Explorer 환경에서 다운로드
	...
	// 3일 전 날짜를 YYYYMMDD 형식으로 구함
	var threeDaysAgo = new Date();
	threeDaysAgo.setDate(today.getDate() - 3);

	// 오늘부터 3일 전까지의 모든 날짜 구함
	var dateRange = [];
	for (var d = new Date(threeDaysAgo); d < today; d.setDate(d.getDate() + 1)) {
	...
	// 날짜 요소를 찾아서 범위 내 날짜에 빨간색 적용
	$('td').each(function() {
		var dateText = $(this).text();
		if (dateRange.includes(dateText)) {
			$(this).css("color","red");
		}
	});
</script>
