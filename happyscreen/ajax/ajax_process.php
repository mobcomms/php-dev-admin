...
Switch($mode) {
	...
	Case 'board_switch': ## 게시판 관리 (ON / OFF)
		...
		$sql = "UPDATE ckd_bbs SET display_yn=:mType WHERE seq=:idx ";
	...
	Case 'android_ver': ## 환경 설정 (앱버전 관리 - 안드로이드)
		...
		$sql = "UPDATE ckd_cfg_info SET cfg_val=:android_ver, alt_dttm=now() WHERE cfg_nm='android_ver' ";
	...
	Case 'mobon_YN': ## 환경 설정 (모비온 연동 상태)
		...
		$sql = "UPDATE ckd_cfg_info SET cfg_val=:mobon_YN, alt_dttm=now() WHERE cfg_nm='mobon_YN' ";
	...
	Case 'typing_game_YN': ## 환경 설정 (타이핑 게임 상태)
		...
		$sql = "UPDATE ckd_cfg_info SET cfg_val=:cfg_val, alt_dttm=now() WHERE cfg_nm='typing_game_YN' ";
	...
	Case 'ad_frequency': ## 환경 설정 (모비온 연동 상태)
		...
		$sql = "UPDATE ckd_cfg_info SET cfg_val=:cfg_val, alt_dttm=now() WHERE cfg_nm='ad_frequency' ";
		$ret = $NDO->sql_query($sql,array(":cfg_val"=>$cfg_val));
		if($ret){
			echo "ok";
		}
	...
	Case 'noti_ad': ## 노티설정 저장
		...
		$sql = "UPDATE ckd_cfg_info SET cfg_val=:cfg_val, alt_dttm=now() WHERE cfg_nm='noti_ad' ";
		$ret = $NDO->sql_query($sql,array(":cfg_val"=>$cfg_val));
		if($ret){
			echo "ok";
		}
	...
	Case 'ad_reward': ## 광고 리워드 설정
		...
		$sql = "UPDATE ckd_cfg_info SET cfg_val=:cfg_val, alt_dttm=now() WHERE cfg_nm='ad_reward' ";
		$ret = $NDO->sql_query($sql,array(":cfg_val"=>$cfg_val));
		if($ret){
			echo "ok";
		}
	...
	Case 'spot_point': ## 스팟성 포인트 ON,OFF
		...
		$sql = " UPDATE ckd_spot_point_set SET use_YN='$use_YN', edit_date=NOW() WHERE code_id='$code_id' ";
		//pre($sql);
		$ret = $NDO->sql_query($sql);
		echo $use_YN;
	break;
	...
	Case 'spot_point_save': ## 스팟성 포인트 저장
		...
		$duplicate_add = "
		 ...
		 ";
		$sql = "
			INSERT INTO ckd_spot_point_set SET {$duplicate_add}, reg_date=NOW()
	...
	Case 'offerwall_show': ## 오퍼월 버튼 설정
		...
		$sql = "UPDATE ckd_cfg_info SET cfg_val=:offerwall_show, alt_dttm=now() WHERE cfg_nm='offerwall_view' ";
	...
	Case 'offerwall_logic': ## 오퍼월 로직 설정
		...
		$sql = "UPDATE ckd_cfg_info SET cfg_val=:offerwall_show, alt_dttm=now() WHERE cfg_nm='offerwall_logic' ";
