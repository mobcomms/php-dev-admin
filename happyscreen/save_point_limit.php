...
if(!empty($_POST)){
	...
	$sql = "
		update ckd_cfg_info SET cfg_val=:new_user, alt_dttm=NOW()
		WHERE cfg_nm = 'new_user_save_point_limit'
	";
	$result1 = $NDO->sql_query($sql,[':new_user'=>$new_user]);

	$old_user = substr($_POST['old_user_save_point_limit'],0,5);
	$sql = "
		update ckd_cfg_info SET cfg_val=:old_user, alt_dttm=NOW()
		WHERE cfg_nm = 'old_user_save_point_limit'
	";
				...
				<div class="panel panel-default">
					...
					<table class="table member_table">
						...
						foreach ($result as $row){
							switch ($row['cfg_nm']){
								case 'new_user_save_point_limit' :
									$title = "신규";
									$input_name = "new_user_save_point_limit";
									$input_value = $row['cfg_val'];
								break;
								case 'old_user_save_point_limit' :
									$title = "신규제외";
